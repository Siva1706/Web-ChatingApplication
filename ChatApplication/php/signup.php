        ...
        if(filter_var($email,FILTER_VALIDATE_EMAIL)){ //if email is valid
          ...
          if(mysqli_num_rows($sql)>0){
            ...
            if(isset($_FILES['image'])){
                ...
                $extensions = ['png','jpg','jpeg'];// these are some valid  img  extensions and  we've stored them in array
                if(in_array($img_ext,$extensions) === true) //if user uploaded img ext is matched with any array extensions
                {
                    $time = time();//this will return us current  time..
                    //we need this time because when you uploads user img to in our folder we rename user file with current time
                    //so all the img file will hava a unique  name
